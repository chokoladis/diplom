...
<head>
    ...
    <title>redmouse.ru/otz.php</title>
    ...
    <section class="first" id="first">
        ...
        <div class="content">
            <div class="container">
                            ...
                            <div class="add_otz">
                                <form action="#" class="d-flex" method="POST">
                                    <select name="select" id="select">
                                        ...
                                        <option value="5" name="5" selected>5</option>
                                    </select>
                                    <input type="text" name="redmouse" id="redmouse" placeholder="О RedMouse">
                                    <input type="text" name="webpage" name="webpage" placeholder="О сайте">
                                    <input type="submit" name="send_otz" id="send_otz" value="Оставить отзыв">
                                </form>
                            </div>
                            ';   
                    $submit = $_POST['send_otz'];
                    $data = $_POST;
                    $redmouse = $data['redmouse'];
                    $webpage = $data['webpage'];
                    $user = $_SESSION['logged_user']->login;
                    $rating = $data['select'];
                    if (isset($submit)) {
                        $query_user = "SELECT `id` FROM `loginpass` WHERE login='$user'";
                        $res_user = mysqli_query($connect,$query_user);
                        while($r = mysqli_fetch_array($res_user)){
                            $user2 = $r['id'];
                        }
                        $query3 = "INSERT INTO `otz` VALUES(NULL,'$user2','$redmouse','$webpage','$rating')";
                ...
                <?
                    $i=0;
                    $query_otz =  mysqli_query($connect,"SELECT * FROM `otz`,`loginpass` WHERE loginpass.id=otz.id_user");
                    $n = mysqli_num_rows($query_otz);
                    while($r = mysqli_fetch_array($query_otz)){
                        if ($i>=$n) break;
                        else {
                            echo "
                            <div class='item'>
                                <div class='user'>{$r[login]}</div>
                                <div class='rating_php'>
                                    <div class='rating_block_item'>
                                        {$r[rating]} / 5
                                    </div>
                                </div>
                                    <div class='main'>
                                        <div class='redmouse'>
                                            <div class='title'>О RedMouse</div>
                                            <div class='info'>{$r[otz_redmouse]}</div>
                                        </div>
                                        <div class='webpage'>
                                            <div class='title'>О сайте</div>
                                            <div class='info'>{$r[otz_webpage]}</div>
                                        </div>
                                    </div>
                            </div>";   
                            $i++;   
